      ...
      (cond
        ...
        ((string= name "format")
         (wrap (lambda (&rest format-args)
                 (flet ((%fmt-apply-spec (val spec-str)
                          (if (zerop (length spec-str))
                              (py-str-of val)
                              (let* ((fill-char #\space) (align nil) (pos 0)
                                     (speclen (length spec-str)))
                                (when (and (>= speclen 2)
                                           (member (char spec-str 1) '(#\< #\> #\^ #\=)))
                                  (setf fill-char (char spec-str 0) align (char spec-str 1))
                                  (incf pos 2))
                                (when (and (null align) (< pos speclen)
                                           (member (char spec-str pos) '(#\< #\> #\^ #\=)))
                                  (setf align (char spec-str pos)) (incf pos))
                                (let ((sign-char nil))
                                  (when (and (< pos speclen)
                                             (member (char spec-str pos) '(#\+ #\- #\space)))
                                    (setf sign-char (char spec-str pos)) (incf pos))
                                  (when (and (< pos speclen) (char= (char spec-str pos) #\#)) (incf pos))
                                  (when (and (< pos speclen) (char= (char spec-str pos) #\0) (null align))
                                    (setf fill-char #\0 align #\=) (incf pos))
                                  (let ((wstr ""))
                                    (loop while (and (< pos speclen) (digit-char-p (char spec-str pos)))
                                          do (setf wstr (concatenate 'string wstr (string (char spec-str pos)))) (incf pos))
                                    (let* ((width (and (plusp (length wstr)) (parse-integer wstr))) (prec nil))
                                      (when (and (< pos speclen) (char= (char spec-str pos) #\.))
                                        (incf pos)
                                        (let ((p ""))
                                          (loop while (and (< pos speclen) (digit-char-p (char spec-str pos)))
                                                do (setf p (concatenate 'string p (string (char spec-str pos)))) (incf pos))
                                          (setf prec (if (string= p "") 0 (parse-integer p)))))
                                      (let* ((tc (and (< pos speclen) (char spec-str pos)))
                                             (formatted
                                               (cond
                                                 ((and tc (member tc '(#\d #\i)))
                                                  (let* ((n (cond ((typep val 'py-int) (py-int-value val))
                                                                  ((typep val 'py-float) (truncate (py-float-value val)))
                                                                  (t 0)))
                                                         (sign (if (and sign-char (eq sign-char #\+) (>= n 0)) "+" ""))
                                                         (s (format nil "~D" n)))
                                                    (concatenate 'string sign s)))
                                                 ((and tc (member tc '(#\f #\F)))
                                                  (let* ((v (cond ((typep val 'py-float) (py-float-value val))
                                                                  ((typep val 'py-int) (float (py-int-value val) 1d0))
                                                                  (t 0d0)))
                                                         (p (or prec 6)))
                                                    (format nil (format nil "~~,~DF" p) v)))
                                                 ((and tc (member tc '(#\e #\E)))
                                                  (let* ((v (if (typep val 'py-float) (py-float-value val)
                                                                (float (or (and (typep val 'py-int) (py-int-value val)) 0) 1d0)))
                                                         (p (or prec 6)))
                                                    (%py-format-exp v p (char= tc #\E))))
                                                 ((and tc (char= tc #\%))
                                                  (let* ((v (if (typep val 'py-float) (py-float-value val)
                                                                (float (or (and (typep val 'py-int) (py-int-value val)) 0) 1d0)))
                                                         (p (or prec 6)))
                                                    (concatenate 'string (format nil (format nil "~~,~DF" p) (* v 100)) "%")))
                                                 (t (let ((sv (py-str-of val)))
                                                      (if prec (subseq sv 0 (min prec (length sv))) sv)))))
                                             (result-str
                                               (if (null width) formatted
                                                   (let* ((flen (length formatted)) (deficit (- width flen)))
                                                     (if (<= deficit 0) formatted
                                                         (let ((pad (make-string deficit :initial-element fill-char)))
                                                           (cond
                                                             ((eq align #\>) (concatenate 'string pad formatted))
                                                             ((eq align #\^)
                                                              (let* ((lp (floor deficit 2)) (rp (- deficit lp)))
                                                                (concatenate 'string (make-string lp :initial-element fill-char)
                                                                             formatted (make-string rp :initial-element fill-char))))
                                                             (t (concatenate 'string formatted pad)))))))))
                                        result-str))))))))
                   (let ((result (make-array 0 :element-type 'character :fill-pointer 0 :adjustable t))
                     ...
                     (loop while (< i len) do
                       (let ((ch (char s i)))
                         (cond
                           ((and (char= ch #\{) (< (1+ i) len) (char= (char s (1+ i)) #\{))
                            (vector-push-extend #\{ result) (incf i 2))
                           ((and (char= ch #\}) (< (1+ i) len) (char= (char s (1+ i)) #\}))
                            (vector-push-extend #\} result) (incf i 2))
                           ((char= ch #\{)
                            (let ((close (position #\} s :start (1+ i))))
                              (if (null close)
                                  (progn (vector-push-extend ch result) (incf i))
                                  (let* ((inner (subseq s (1+ i) close))
                                         (colon (position #\: inner))
                                         (field (if colon (subseq inner 0 colon) inner))
                                         (spec  (if colon (subseq inner (1+ colon)) ""))
                                         (val (cond
                                                ((string= field "")
                                                 (let ((v (if (< auto-idx (length format-args))
                                                              (nth auto-idx format-args) +py-none+)))
                                                   (incf auto-idx) v))
                                                (t (let ((n (ignore-errors (parse-integer field))))
                                                     (if n
                                                         (if (< n (length format-args)) (nth n format-args) +py-none+)
                                                         +py-none+))))))
                                    (loop for c across (%fmt-apply-spec val spec)
                                          do (vector-push-extend c result))
                                    (setf i (1+ close))))))
                           (t (vector-push-extend ch result) (incf i)))))
                     (make-py-str (coerce result 'string)))))))
