      ...
      (let ((fname (clython.ast:name-node-id func-node)))
        ...
        (when (and (string= fname "locals")
          ...
          (let ((d (clython.runtime:make-py-dict)))
            ...
            (return-from eval-node d)))
        ;; dir() — return sorted list of names in current scope
        (when (and (string= fname "dir")
                   (null (clython.ast:call-node-args node)))
          (let ((names nil))
            ;; Collect all names from all scope levels
            (loop with e = env
                  while e do
                  (maphash (lambda (k _v)
                             (declare (ignore _v))
                             (pushnew k names :test #'string=))
                           (clython.scope:env-bindings e))
                  (setf e (clython.scope:env-parent e)))
            (let* ((sorted (sort names #'string<))
                   (py-names (mapcar #'clython.runtime:make-py-str sorted)))
              (return-from eval-node
                (clython.runtime:make-py-list (coerce py-names 'vector)))))
          (return-from eval-node (clython.runtime:make-py-list #())))
        ...
        (when (and (string= fname "eval")
          ...
          (let* ((evaluated-args (mapcar (lambda (a) (eval-node a env))
                                         (clython.ast:call-node-args node)))
                 (src (clython.runtime:py-str-value (first evaluated-args)))
                 (ns-dict (when (>= (length evaluated-args) 2)
                            (second evaluated-args)))
                 (eval-env (if (and ns-dict
                                    (typep ns-dict 'clython.runtime:py-dict))
                               ;; Build a fresh env seeded from the namespace dict
                               (let ((e (make-instance 'clython.scope:environment)))
                                 (maphash (lambda (k v)
                                            (when (stringp k)
                                              (clython.scope:env-set k v e)))
                                          (clython.runtime:py-dict-value ns-dict))
                                 e)
                               env))
            ...
            (if (and (= (length body) 1)
                ...
                (return-from eval-node
                  (eval-node (clython.ast:expr-stmt-node-value (first body)) eval-env))
                ;; Otherwise evaluate all statements, return last result
                (let ((result clython.runtime:+py-none+))
                  (dolist (stmt body)
                    (setf result (eval-node stmt eval-env)))
                  (return-from eval-node result)))))
        ;; exec(code_string[, globals[, locals]]) — parse and execute statements
        (when (and (string= fname "exec")
          ...
          (let* ((evaluated-args (mapcar (lambda (a) (eval-node a env))
                 ...
                 (tokens (clython.lexer:tokenize src))
                 (ast (clython.parser:parse-module tokens)))
            (eval-node ast exec-env)
            ;; If a namespace dict was provided, sync back new bindings into it
            (when (and ns-dict (typep ns-dict 'clython.runtime:py-dict))
              (maphash (lambda (k v)
                         (unless (eq v clython.runtime:+py-deleted+)
                           (setf (gethash k (clython.runtime:py-dict-value ns-dict)) v)))
                       (clython.scope:env-bindings exec-env)))
    ...
    (dolist (handler handlers)
      (unless handled
        ...
        (when (%exception-matches-handler-p
          ...
          (let ((exc-var-name (clython.ast:exception-handler-name handler)))
            (when exc-var-name
              (clython.scope:env-set exc-var-name exc-val env))
            (dolist (stmt (%sort-body (clython.ast:exception-handler-body handler)))
              (eval-node stmt env))
            ;; CPython deletes the exception variable after the handler body
            ;; (PEP 3110 — prevents reference cycles)
            (when exc-var-name
              (clython.scope:env-del exc-var-name env)))
          (setf handled t))))
    handled))
